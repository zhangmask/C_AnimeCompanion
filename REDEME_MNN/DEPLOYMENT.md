# Anime Companion 部署指南 (MNN 版本)

> **本版本使用 MNN 推理框架 + Qwen3.5 2B 模型。**
...
| Android NDK | r27c | 用于编译 native 代码 (MNN, stable-diffusion.cpp) |
| CMake | 3.22.1+ | NDK 自带 |
...
### 4.2 推送 MNN 主模型（Qwen3.5 2B）

```bash
# INT4 量化版本（默认，推荐）
adb push models/qwen3.5-2b-int4.mnn \
    /sdcard/Android/data/com.companion.chat/files/models/

# FP16 高精度版本（可选，高端设备用）
adb push models/qwen3.5-2b-fp16.mnn \
    /sdcard/Android/data/com.companion.chat/files/models/
```

### 4.3 推送 MNN 文本嵌入模型（生图用）

```bash
adb push models/qwen3.5-2b-embedding.mnn \
    /sdcard/Android/data/com.companion.chat/files/models/
```

### 4.4 推送 MOSS TTS 语音合成模型
...
1. 确认模型文件完整（INT4 版本约 1.2GB）
2. 确认手机架构是 arm64-v8a
3. 检查 `/sdcard/Android/data/com.companion.chat/files/models/` 目录下有 `.mnn` 文件

---

## 与 Gemma 4 版本的区别

| 项目 | Gemma 4 版本 | MNN 版本 |
|------|-------------|----------|
| 主模型 | Gemma 4 E2B (.gguf/.litertlm) | Qwen3.5 2B (.mnn) |
| 推理框架 | llama.cpp + LiteRT-LM | MNN |
| 模型大小 | 1.5-2.4GB | 1.2GB (INT4) |
